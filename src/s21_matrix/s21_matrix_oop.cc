...
// TODO: в описаниях на русском заменить перенос на перемещение
 ...
 * переноса (см. описание конструктора переноса). А еще есть концепция, что
 * дефолтный конструктор должен создавать "пустой" объект (см. класс
...
// TODO: написать про инварианты самого языка и noexcept
/**
 * @brief
 ...
 *
 * Как видно по коду, матрица other становится [0 × 0], а т.к. после мува объект
 * должен оставаться консистентным (см. описание присваивания переносом), то наш
 * класс должен работать с матрицами [0 × 0], поэтому мы (в т.ч.) разрешаем
 * создавать такие матрицы и учитываем существование пустых матриц во всех
 * методах класса.
 *
